...
def _is_special_link(path: Path) -> bool:
    """Reject symlinks and Windows reparse-point directory junctions."""
...
def _scan_role_space(
    *,
    ...
    if not role_space.is_dir() or _is_special_link(role_space):
        records.append(
            _finding_record(
                root=root,
                state_version=state_version,
    ...
    *,
    ...
    for child in sorted(module_root.iterdir(), key=lambda path: path.name):
        ...
        if name in MODULE_RESERVED_ITEM_NAMES:
            ...
            if not child.is_dir() or _is_special_link(child):
                records.append(
                    _finding_record(
                        root=root,
                        state_version=state_version,
...
def _scan_module_vendor_workspace(
    *,
    ...
    if not vendor_root.is_dir() or _is_special_link(vendor_root):
        records.append(
            _finding_record(
                root=root,
                state_version=state_version,
    ...
    for project in sorted(vendor_root.iterdir(), key=lambda path: path.name):
        ...
        if not project.is_dir() or _is_special_link(project):
            records.append(
                _finding_record(
                    root=root,
                    state_version=state_version,
...
def _scan_vendor_role_space(
    *,
    ...
    for module_root in sorted(role_space.iterdir(), key=lambda path: path.name):
        ...
        if not module_root.is_dir() or _is_special_link(module_root):
            records.append(
                _finding_record(
                    root=root,
                    state_version=state_version,
...
def _validate_embedded_workspace_carrier(
    *,
    ...
    if not workspace_root.is_dir() or _is_special_link(workspace_root):
        records.append(
            _finding_record(
                root=root,
                state_version=state_version,
    ...
    for child in children:
        ...
        if (
            child.is_dir()
            and not _is_special_link(child)
...
def _validate_free_remainder_carrier(
    *,
    ...
    if carrier.is_file() and not _is_special_link(carrier):
        return
    if carrier.is_dir() and not _is_special_link(carrier):
...
def _validate_free_remainder_directory(
    *,
    ...
    for child in children:
        ...
        if child.is_file() and not _is_special_link(child):
            continue
        if child.is_dir() and not _is_special_link(child):
            _validate_free_remainder_directory(
                root=root,
                state_version=state_version,
                excluded=excluded,
    ...
    *,
    ...
    for child in children:
        ...
        if allow_data_test and inside_data_root and child.name.lower() == "test":
            if child.name != "Test" or not child.is_dir() or _is_special_link(child):
                records.append(
                    _finding_record(
                        root=root,
                        state_version=state_version,
...
def _scan_src(
    *,
    ...
    for child in sorted(src_root.iterdir(), key=lambda path: path.name):
        ...
        if name not in SRC_AREA_NAMES or not child.is_dir() or _is_special_link(child):
            records.append(
                _finding_record(
                    root=root,
                    state_version=state_version,
...
def _scan_import_files(
    *,
    ...
    for child in sorted(import_root.iterdir(), key=lambda path: path.name):
        ...
        if child.is_dir() or _is_special_link(child) or not child.is_file():
            records.append(
                _finding_record(
                    root=root,
                    state_version=state_version,
...
def _check_test_mirrors(
    *,
    ...
    for relative_path, source_stem, suffix in source_files:
        ...
        if expected_path.is_file() and not _is_special_link(expected_path):
            continue
        records.append(
            _finding_record(
                root=root,
...
def scan_product_project_root(root: str | Path) -> list[dict[str, Any]]:
    ...
    if not root_path.exists() or not root_path.is_dir() or _is_special_link(root_path):
