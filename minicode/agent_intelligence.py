...
from minicode.types import ToolCall
...
class ToolScheduler:
    ...
    def __init__(
        ...
        self._force_max_workers: int | None = None
        self._pending_skill_update = False
        self._force_tool_timeout: float | None = None
        self._force_nudge_frequency = False

    def schedule_calls(self, calls: list[ToolCall], tools: Any) -> tuple[list[ToolCall], list[ToolCall]]:
        ...
        scored_calls: list[tuple[float, ToolCall]] = []
        for call in calls:
    ...
    def _has_conflicts(self, tool_name: str, concurrent_calls: list[ToolCall]) -> bool:
    ...
    def _count_conflicts(self, calls: list[ToolCall]) -> int:
        count = 0
