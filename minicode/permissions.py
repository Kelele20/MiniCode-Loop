...
class PermissionManager:
    ...
    def ensure_network_request(self, method: str, url: str) -> None:
        """Require an explicit one-shot approval for state-changing HTTP requests."""

        normalized_method = method.strip().upper()
        if normalized_method in {"GET", "HEAD"}:
            return
        signature = f"{normalized_method} {url}"
        assessment = self.auto_checker.assess_risk(
            "http_request",
            {"method": normalized_method, "url": url},
        )
        if assessment.action == "approve":
            get_mode_state().record_decision("approve")
            log_permission_check("http_request", signature, granted=True)
            return
        if assessment.action == "block":
            get_mode_state().record_decision("block")
            log_permission_check("http_request", signature, granted=False)
            raise RuntimeError(f"Network request blocked: {assessment.reason}")
        if self.prompt is None:
            log_permission_check("http_request", signature, granted=False)
            raise RuntimeError(
                f"Network request requires approval: {signature}. "
                "Start minicode in TTY mode to approve it."
            )
        result = self.prompt(
            {
                "kind": "network",
                "summary": "mini-code wants to send a state-changing network request",
                "details": [f"method: {normalized_method}", f"url: {url}"],
                "scope": signature,
                "choices": [
                    {"key": "y", "label": "allow once", "decision": "allow_once"},
        ...
        if result.get("decision") == "allow_once":
            ...
            return
        log_permission_check("http_request", signature, granted=False)
        raise RuntimeError(f"Network request denied: {signature}")
