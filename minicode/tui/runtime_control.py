...
def exit_tty_runtime(prev_sigwinch: SignalHandler) -> None:
    if prev_sigwinch is not None and sys.platform != "win32":
        import signal as _signal

        sigwinch = getattr(_signal, "SIGWINCH", None)
        if sigwinch is not None:
            _signal.signal(sigwinch, prev_sigwinch)
    show_cursor()
    exit_alternate_screen()


def install_sigwinch_rerender(throttled: _ThrottledRenderer) -> SignalHandler:
