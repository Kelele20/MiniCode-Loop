    ...
    def __enter__(self) -> _RawModeContext:
        if sys.platform == "win32":
            ...
            try:
                ...
                logger.debug("Could not configure Windows console encoding", exc_info=True)
        else:
            import termios
            import signal
        ...
        if sys.platform == "win32":
            if self._old_cp is not None:
                try:
                    ...
                    logger.debug("Could not restore Windows console encoding", exc_info=True)
            ...
            if getattr(self, '_old_sigwinch', None) is not None:
                try:
                    ...
                    logger.debug("Could not restore SIGWINCH handler", exc_info=True)
