...
def _render_banner(runtime: dict | None, cwd: str, permission_summary: list[str], counts: dict[str, int]) -> str:
    ...
    lines = [
        "╔══════════════════════════════════════════════════════════╗",
        "║  🤖 MiniCode Loop - Your Terminal Coding Assistant        ║",
        "╠══════════════════════════════════════════════════════════╣",
        f"║  Model: {model:<46} ║",
        f"║  CWD: {cwd:<50} ║",
    ]
...
def _configure_stdio_for_unicode() -> None:
    ...
    if stdin_reconfigure is not None:
        try:
            ...
            logger.debug("Could not reconfigure stdin encoding", exc_info=True)
    ...
    for stream in (sys.stdout, sys.stderr):
        ...
        if reconfigure is not None:
            try:
                ...
                logger.debug("Could not reconfigure output stream encoding", exc_info=True)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="MiniCode Loop - A self-regulating terminal coding assistant",
        add_help=True,
    )
    parser.add_argument(
        "--resume",
