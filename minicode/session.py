...
def _deserialize_chat_message(data: dict[str, Any]) -> ChatMessage:
    """Load only fields supported by the public chat-message contract."""
    message: ChatMessage = {}
    role = data.get("role")
    if role == "system":
        message["role"] = "system"
    ...
    for key in ("toolUseId", "toolName"):
        value = data.get(key)
        if isinstance(value, str):
            if key == "toolUseId":
                message["toolUseId"] = value
            else:
                message["toolName"] = value
    if "input" in data:
...
class SessionData:
    ...
    metadata: SessionMetadata = field(
        default_factory=lambda: SessionMetadata("", 0.0, 0.0)
    )
...
def _load_session_from_bundle(session_id: str) -> SessionData | None:
    ...
    session = SessionData(
        ...
        updated_at=updated_at,
        workspace=record.metadata.cwd,
        messages=[_deserialize_chat_message(message) for message in record.messages],
