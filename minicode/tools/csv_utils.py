    ...
    try:
        ...
        if isinstance(data, list) and len(data) > 0:
            if isinstance(data[0], dict):
                # List of dicts
                if include_header:
                    dict_writer = csv.DictWriter(output, fieldnames=data[0].keys(), delimiter=delimiter)
                    dict_writer.writeheader()
                    dict_writer.writerows(data)
                else:
                    row_writer = csv.writer(output, delimiter=delimiter)
                    row_writer.writerow(list(data[0].keys()))
                    for row in data:
                        row_writer.writerow(row.values())
            else:
                # List of lists
                list_writer = csv.writer(output, delimiter=delimiter)
