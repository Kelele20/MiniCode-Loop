    ...
    try:
        ...
        if store:
            try:
                ...
                logger.debug("Could not restore idle state after tool failure", exc_info=True)
    ...
    if control_signal.suggest_memory_persistence:
        ...
        if context_compactor and hasattr(context_compactor, "_tool_budget"):
            try:
                ...
                logger.debug("Tool budget flush failed", exc_info=True)
...
def run_agent_turn(
    *,
    ...
    max_steps = runtime_profile.max_steps or max_steps
    turn_state.max_steps = max_steps
    ...
    def _on_assistant_message(content: str) -> None:
        publish_turn_event(
    ...
    def _on_progress_message(content: str) -> None:
        publish_turn_event(
    ...
    def _on_runtime_event(event: RuntimeEvent) -> None:
        publish_turn_event(
    ...
    def _on_assistant_stream_chunk(content: str) -> None:
        publish_turn_event(
    ...
    def _on_thinking_chunk(content: str) -> None:
        publish_turn_event(
    ...
    on_tool_start = _on_tool_start if callbacks is not None or legacy_on_tool_start else None
    on_tool_result = _on_tool_result if callbacks is not None or legacy_on_tool_result else None
    on_assistant_message = (
        _on_assistant_message if callbacks is not None or legacy_on_assistant_message else None
    )
    on_progress_message = (
        _on_progress_message if callbacks is not None or legacy_on_progress_message else None
    )
    on_runtime_event = _on_runtime_event if callbacks is not None or legacy_on_runtime_event else None
    ...
    if (
        ...
        on_assistant_stream_chunk = _on_assistant_stream_chunk
    if (
    ...
    if enable_work_chain:
        ...
        if smart_router and prelude.task:
            try:
                ...
                logger.debug("Smart routing initialization failed", exc_info=True)
        ...
        if prelude.task:
            ...
            if preemptive_config.confidence > 0.6:
                ...
                max_steps = turn_state.max_steps or max_steps
                logger.info(
        ...
        if model_selection_ctrl and prelude.task:
            try:
                ...
                logger.debug("Model-selection decision failed", exc_info=True)
        ...
        if context_manager:
            ...
            try:
                ...
                logger.debug("Memory reranker initialization failed", exc_info=True)
            ...
            if memory_injection_ctrl:
                try:
                    ...
                    logger.debug("Memory injection control failed", exc_info=True)
            ...
            if orch and prelude.task:
                try:
                    ...
                    logger.warning("Orchestrated memory injection failed", exc_info=True)
    ...
    try:
        ...
        while turn_state.has_remaining_steps():
            ...
            try:
                ...
                if model_switcher and "rate" not in error_str and _should_attempt_model_fallback(error_str):
                    try:
                        ...
                        logger.debug("Provider failure metric update failed", exc_info=True)
            ...
            _results: list[tuple[ToolCall, ToolResult]] = []
                ...
                if orch:
                    ...
                    if progress_controller:
                        progress_signal = ProgressSignal(
                            total_steps=turn_state.max_steps or max_steps,
        ...
        if enable_work_chain and prelude.task:
            ...
            if orch and prelude.task:
                try:
                    ...
                    logger.warning("Orchestrated task reflection failed", exc_info=True)
                ...
                try:
                    ...
                    logger.warning("Task reflection failed", exc_info=True)
            ...
            if memory_injector and hasattr(memory_injector, '_cached_result'):
                try:
                        ...
                        for scope_name in ['project', 'local', 'user']:
                            try:
                                ...
                                if scope in memory_mgr.memories:
                                    mem_store = memory_mgr.memories[scope]
                    ...
                    logger.debug("Memory quality feedback failed", exc_info=True)
            ...
            if smart_router and prelude.task:
                try:
                    ...
                    logger.debug("Router feedback learning failed", exc_info=True)
        ...
        if cost_control:
            ...
            stats_adjustment = cc_stats.get("adjustment")
            logger.info(
                ...
                stats_adjustment["budget_mult"] if stats_adjustment else 1.0,
                stats_adjustment["threshold_mult"] if stats_adjustment else 1.0,
                stats_adjustment["reason"] if stats_adjustment else "none",
            )
        ...
        if context_cybernetics and feedback_controller:
            ...
            if adaptive_pid_tuner and step > 0 and step % 20 == 0 and feedback_controller:
                try:
                    ...
                    logger.debug("Adaptive PID final tuning failed", exc_info=True)
