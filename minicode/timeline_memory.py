...
class StateReasoner:
    ...
    def _range_numeric_records(
        ...
        *,
        ...
        values: list[tuple[float, StateRecord]] = []
        for record in records:
            value = _parse_number(record.value)
...
def build_timeline_context(
    *,
    ...
    seen = set()
    for timeline_turn in [*latest_candidates, *chronological]:
        key = (timeline_turn.session_id, timeline_turn.turn_index)
        if key in seen:
            continue
        seen.add(key)
        selected.append(timeline_turn)
    ...
    for timeline_turn in latest_candidates:
        lines.append(
            f"- [{timeline_turn.session_date} {timeline_turn.session_id} turn {timeline_turn.turn_index} "
            f"{timeline_turn.role} rel={timeline_turn.relevance:.3f}] {timeline_turn.content}"
        )
    lines.extend(["", "Chronological evidence:"])

    used = sum(len(line) + 1 for line in lines)
    formatted_turns: list[TimelineTurn] = []
    for timeline_turn in chronological:
        line = (
            f"- [{timeline_turn.session_date} {timeline_turn.session_id} turn {timeline_turn.turn_index} "
            f"{timeline_turn.role} rel={timeline_turn.relevance:.3f}] {timeline_turn.content}"
        )
        ...
        lines.append(line)
        used += len(line) + 1
        formatted_turns.append(timeline_turn)
