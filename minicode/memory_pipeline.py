...
class MemoryPipeline:
    ...
    def __init__(self, memory_manager: Any | None = None):
        self._memory: Any = memory_manager
    ...
    def initialize(
        ...
        if enable_vector:
            try:
                ...
                logger.warning("Vector memory initialization failed", exc_info=True)
    ...
    def read(
        ...
        if self._vector_store and self._vector_store.enabled:
            try:
                ...
                logger.debug("Vector memory lookup failed", exc_info=True)
        ...
        if self._reranker and self._reranker.enabled and len(entries) > 3:
            try:
                ...
                logger.debug("Memory reranking failed; using BM25 results", exc_info=True)
    ...
    def inject(
        ...
        if self._last_inject_time and now - self._last_inject_time < cooldown:
    ...
    def write(
        ...
        try:
            ...
            logger.warning("Memory write pipeline failed", exc_info=True)
    ...
    def _graph_query_policy(cls, query: str) -> dict[str, Any]:
        ...
        return {
            "intent": intent,
            "relation_weights": dict(policy["relation_weights"] if isinstance(policy["relation_weights"], dict) else {}),
    ...
    def _merge_graph_candidates(
        ...
        *,
        ...
        for entry_id in candidate_ids:
            ...
            if hit is not None and getattr(hit, "relations", False):
                graph_score = min(
                    1.0,
