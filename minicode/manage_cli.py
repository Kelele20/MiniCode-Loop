...
def _handle_mcp_command(cwd: str, args: list[str]) -> bool:
    ...
    if subcommand == "add":
        ...
        name = head.pop(0)
        add_protocol = _take_option(head, "--protocol")
...
def _handle_skills_command(cwd: str, args: list[str]) -> bool:
    ...
    if subcommand == "remove":
        ...
        remove_result = remove_managed_skill(cwd, rest[0], scope=scope)
        if not remove_result["removed"]:
            print(f"Skill {rest[0]} not found at {remove_result['targetPath']}")
            return True
        print(f"Removed skill {rest[0]} from {remove_result['targetPath']}")
        return True
    _print_usage()
    return True
