...
If `MiniCode-Loop` is mounted into the main `MiniCode` repository as a submodule, then the main repository does **not** automatically track the latest state of this repository.
...
git commit -m "Update MiniCode Loop submodule"
git push
```
...
cd ..
git add <python-submodule-path>
git commit -m "Pin MiniCode Loop submodule to <exact-commit>"
git push
```
...
1. finish changes in `MiniCode-Loop`
...
1. confirm the target commit exists in the Python repository
