...
RELEASE_STATE_HOME = REPO_ROOT / ".temp" / "release-home"
...
def _run_command(label: str, command: list[str], *, cwd: Path, timeout: int = 1800) -> ReleaseCheck:
    ...
    try:
        # Config is read at import time from Path.home().  Every child shares a
        # project-local state home so session smokes are coherent while real
        # user settings and credentials remain unreachable.
        RELEASE_STATE_HOME.mkdir(parents=True, exist_ok=True)
        env["HOME"] = str(RELEASE_STATE_HOME)
        env["USERPROFILE"] = str(RELEASE_STATE_HOME)
...
def _offline_provider_diagnostic(runtime_eval_check: ReleaseCheck) -> dict[str, object]:
    """Describe missing provider evidence without starting a provider-capable process."""
    summary = (
        "Live provider smoke was not run, and the offline runtime profile evaluation "
        "did not produce a provider diagnostic."
    )
    ...
    return {
        "label": "offline-provider-diagnostic",
        "outcome": "provider_channel_unavailable",
        "command": "offline release readiness",
        "exit_code": runtime_eval_check.exit_code,
        "summary": summary,
        "error_code": "",
        "request_id": "",
        ...
        "trace_artifact": "",
        "stdout": "",
        "stderr": runtime_eval_check.stderr,
    }
...
def _prepare_saved_session(workspace: Path) -> None:
    import minicode.session as session_store

    state_dir = RELEASE_STATE_HOME / ".mini-code"
    session_store.MINI_CODE_DIR = state_dir
    session_store.SESSIONS_DIR = state_dir / "sessions"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if RELEASE_STATE_HOME.exists():
        shutil.rmtree(RELEASE_STATE_HOME)
    RELEASE_STATE_HOME.mkdir(parents=True, exist_ok=True)
    ...
    if not provider_diagnostics:
        provider_diagnostics = [_offline_provider_diagnostic(runtime_eval_check)]

    readiness_report = build_readiness_report(
        REPO_ROOT,
        runtime={"model": "offline-release-evidence"},
    )
